...
==========================
Technical indicators and feature generation for trading models.
...
def add_technical_indicators_multi_tf(
    ...
    """
    Add technical indicators for multi-timeframe aligned data.

    Expects DataFrame with columns suffixed by timeframe:
    - open_1d, high_1d, low_1d, close_1d, volume_1d
    - open_1wk, high_1wk, low_1wk, close_1wk, volume_1wk
    - open_1mo, high_1mo, low_1mo, close_1mo, volume_1mo

    Args:
        df: Aligned multi-timeframe DataFrame
        timeframes: List of timeframe suffixes
...
def prepare_features_for_env_multi_tf(
    ...
    """
    Prepare and select features for multi-timeframe trading environment.
...
if __name__ == "__main__":
    ...
    console.print("[bold]Testing Feature Engineering[/bold]")
    ...
    # Create aligned data (simulating 3 timeframes)
    df = pd.DataFrame(index=dates)

    for tf in ['1d', '1wk', '1mo']:
        df[f'open_{tf}'] = price + np.random.randn(n) * 0.05
        df[f'high_{tf}'] = price + abs(np.random.randn(n) * 0.1)
        df[f'low_{tf}'] = price - abs(np.random.randn(n) * 0.1)
        df[f'close_{tf}'] = price + np.random.randn(n) * 0.05
    ...
    # Show stats for first timeframe
    stats = get_feature_stats_multi_tf(df_prepared, feature_map)
    console.print("\n[bold]Feature Statistics (1d):[/bold]")
    console.print(stats['1d'].to_string())
